...
export const exportToPdf = (currentDate: Date, schedule: DaySchedule[], workers: Worker[]) => {
  ...
  doc.setFontSize(16)
  doc.text(`${monthName} ${year} - Restaurant Staff Schedule`, pageWidth / 2, margin + 10, { align: "center" })
  ...
  const columns = [
    ...
    { header: "Day", width: dayNameColWidth },
    { header: "Cashier 1", width: positionColWidth },
    { header: "Cashier 2", width: positionColWidth },
    { header: "Cashier 3", width: positionColWidth },
    { header: "Kitchen 1", width: positionColWidth },
    { header: "Kitchen 2", width: positionColWidth },
    { header: "Kitchen 3", width: positionColWidth },
    { header: "Kitchen 4", width: positionColWidth },
  ]
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (rowY + rowHeight > pageHeight - margin) {
      ...
      doc.setFontSize(16)
      doc.setFont("helvetica", "bold")
      doc.text(`${monthName} ${year} - Restaurant Staff Schedule (continued)`, pageWidth / 2, margin + 10, {
        align: "center",
      })
  ...
  for (let i = 0; i < totalPages; i++) {
    ...
    doc.setFontSize(10)
    doc.setFont("helvetica", "normal")
    doc.text(`${i + 1} of ${totalPages}`, margin, margin, { align: "left" })
  }

  // Save PDF
  doc.save(`restaurant-schedule-${monthName.toLowerCase()}-${year}.pdf`)
}
...
export const exportToExcel = (currentDate: Date, schedule: DaySchedule[], workers: Worker[]) => {
  ...
  // Save Excel file
  XLSX.writeFile(wb, `restaurant-schedule-${monthName.toLowerCase()}-${year}.xlsx`)
}
